import { Town } from '@/data/types';

// Source: RecycleCoach API (recyclecoach.com)
// Scraped: 2026-03-25 — auto-generated by scripts/scrape-recyclecoach.ts
//
// TODOs before this file is production-ready:
//   1. Verify schedule accuracy against the municipal website
//   2. Add recyclingCenter (address, hours, phone)
//   3. Add specialInstructions from the DPW page
//   4. Add street-to-zone assignments if the town has multiple zones
//   5. Check yard waste season dates

export const hasbrouckHeights: Town = {
  id: 'nj-bergen-hasbrouck-heights',
  name: 'Hasbrouck Heights',
  state: 'NJ',
  county: 'Bergen',

  zones: [
    // Zone 1 (RecycleCoach zone: zone-z13815)
    {
      id: 'hasbrouckHeights-zone-1',
      name: 'Zone 1',
      description: 'Garbage collected Monday & Wednesday & Thursday',
      streets: [], // TODO: Street assignments not available via RecycleCoach
      schedule: {
        garbage: {
          days: [1, 3, 4], // Monday, Wednesday, Thursday
          time: '6:00 AM', // TODO: verify curb-out time
        },
        recycling: {
          type: 'alternating',
          day: 3, // Wednesday
          weeks: {
            even: 'commingled',
            odd: 'paper',
          },
          evenLabel: 'Commingled',
          oddLabel: 'Paper',
        },
        yardWaste: {
          days: [4], // Thursday
          seasonStart: '01-01',
          seasonEnd: '12-30',
        },
      },
    },

    // Zone 2 (RecycleCoach zone: zone-z13816)
    {
      id: 'hasbrouckHeights-zone-2',
      name: 'Zone 2',
      description: 'Garbage collected Tuesday & Friday',
      streets: [], // TODO: Street assignments not available via RecycleCoach
      schedule: {
        garbage: {
          days: [2, 5], // Tuesday, Friday
          time: '6:00 AM', // TODO: verify curb-out time
        },
        recycling: {
          type: 'alternating',
          day: 3, // Wednesday
          weeks: {
            even: 'commingled',
            odd: 'paper',
          },
          evenLabel: 'Commingled',
          oddLabel: 'Paper',
        },
        yardWaste: {
          days: [5], // Friday
          seasonStart: '01-01',
          seasonEnd: '12-30',
        },
      },
    }
  ],

  // recyclingCenter: {
  //   name: 'Hasbrouck Heights Recycling Center',
  //   address: 'TODO',
  //   hours: { weekday: 'TODO', saturday: 'TODO' },
  //   phone: 'TODO',
  // },

  // specialInstructions: [],
};
